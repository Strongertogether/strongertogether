...
function upload_files() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/workspace/media/'.$_FILES['avatar']['name'];
    ...
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/workspace/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    }
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = '/workspace/media/default-avatar.png';
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
                ...
                function remove_files(){
                	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/workspace/media/'.$_SESSION['nombreFichero'])){
                		unlink($_SERVER['DOCUMENT_ROOT'].'/workspace/media/'.$_SESSION['nombreFichero']);
                		return true;
                	}else{
                		return false;
                	}
